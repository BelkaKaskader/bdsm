...
const DataCharts: React.FC<DataChartsProps> = ({ data }) => {
  ...
  const prepareChartData = () => {
    const totalFund = data.reduce((sum, row) => sum + row.Сумма_по_полю_ФОТт, 0);
    const sortedData = [...data].sort((a, b) => {
      if (selectedChart === 'workforce') {
        ...
        return b.Сумма_по_полю_ФОТт - a.Сумма_по_полю_ФОТт;
      }
    });

    return sortedData.slice(0, 10).map(row => {
      const fundPercentage = (row.Сумма_по_полю_ФОТт / totalFund) * 100;
      return {
